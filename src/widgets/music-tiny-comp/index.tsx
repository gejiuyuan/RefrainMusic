		...
		return () => {
			...
			return (
				<div class="play-order controller-widget" ref={playOrderRef}>
					<div
						className="play-order-noumenon controller-widget-noumenon"
						onClick={noumenonClick}
						title={PlayOrderInfo[order]}
					>
					...
					<div
						className="play-order-suspension controller-widget-suspension"
						visibility={suspensionShow.value}
					>
						<div class="play-order-duplicate controller-widget-duplicate">
							<div
								className="play-order-duplicate-item"
								onClick={() => switchOrderStatus('random')}
							>
							...
							<div
								className="play-order-duplicate-item"
								onClick={() => switchOrderStatus('order')}
							>
							...
							<div
								className="play-order-duplicate-item"
								onClick={() => switchOrderStatus('singleLoop')}
							>
...
export const Volume = defineComponent({
	...
	setup(props, { slots, emit }) {
		...
		onKeyStroke(
			isCtrlAndArrowUp,
			() => {
				let tarVolume = volumeRefGlobal.value + 0.05;
		...
		return () => {
			...
			return (
				<div class="volume controller-widget" ref={volumeRef}>
					<div
						className="volume-noumenon controller-widget-noumenon"
						onClick={switchShow}
						title={title}
					>
					...
					<div
						className="volume-suspension controller-widget-suspension"
					...
					>
						...
						<div
							class="volume-duplicate controller-widget-duplicate"
							onClick={switchMuted}
						>
							<i className="iconfont icon-yinliang" hidden={mute}></i>
							<i className="iconfont icon-mute" hidden={!mute}></i>
			...
			if (!id) {
				...
				if (code == 200) {
					...
					messageBus.dispatch(
						...
						`${
							willIsLovedValue
								? `喜欢成功${UNICODE_CHAR.hugface}`
								: `已移除${UNICODE_CHAR.pensive}`
						}`,
					);
				}
			}
			...
			return (
				<p class={classNameObj} singallinedot>
					{singers.map(({ id, name }, i) => {
						return (
							<>
								<span onClick={() => toSingerDetailPage(id)} title={name}>
									{name}
								</span>
