/**
 * @format
 * @copyfrom element-plus
 */

import { nextTick } from 'vue';
import type { ObjectDirective, ComponentPublicInstance } from 'vue';
import { throttle, is, getOffsetTopDistance, getScrollContainer } from '@utils/index';

export const SCOPE = 'YuanInfiniteScroll';
...
type ScrollOptions = { [K in keyof Attrs]: Attrs[K]['default'] } & PlainObject;
...
const VInfiniteScroll: ObjectDirective<InfiniteScrollEl, InfiniteScrollCallback> = {
	async mounted(el, binding) {
		...
		if (!is.function(cb)) {
			console.error("'v-infinite-scroll' binding value must be a function");
		}
		...
		const containerEl =
			container === window ? document.documentElement : (container as HTMLElement);
