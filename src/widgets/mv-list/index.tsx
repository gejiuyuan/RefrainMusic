...
import { getLocaleCount, getLocaleDate, msSecondToTimeStr, padPicCrop, second2TimeStr } from "@utils/index";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const renderMvLists = () => {
      const { gaps: { x, y }, cols, mvlists } = props;
      ...
      return (
        <NGrid class="mv-list" xGap={x} yGap={y} cols={cols}>
          {
            mvlists.map(({ name, imgurl16v9, cover, duration, id, publishTime, playCountStr }) =>
              <NGridItem key={id}>
                <div class="mv-item" onClick={() => toMvDetailPage(id)}>
                  ...
                  <div class="desc">
                    <p class="duration">
                      <span>时长：</span>
                      <em>
                        {msSecondToTimeStr(duration)}
                      </em>
                    </p>
                    {
                      publishTime && <p class="publishtime">
                        <span>发布：</span>
                        <em>
                          {publishTime}
                        </em>
                  ...
                  {renderMvLists()}
                  {
                    props.showPagination && (
                      <section class="songlist-pagination">
                        <RoutePagination pagiInfo={mvListPagiInfo} hasMore={props.hasMore}></RoutePagination>
                      </section>
                    )
                  }
                </section>
