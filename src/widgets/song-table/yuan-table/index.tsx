...
import { EMPTY_ARR, is } from '@utils/index';
...
const defaultComp = defineComponent({
	...
	setup(props, { slots, emit }) {
		const { linkChildren } = useChildren<ComponentPublicInstance, ParentExtraData>(
			YUAN_TABLE_KEY,
		);
		...
		return () => {
			...
			const slotArr = slots.default!().filter(
				({ type }) => is.object(type) && Reflect.get(type, 'name'),
			);
			const totalSpan = slotArr
				.map((slot) => slot!.props?.span || 1)
				.reduce((a, b) => a + b);
				...
				<section class={`yuan-table ${className}`}>
					<header class="yuan-table-header">
						<div class="yuan-table-layer header-layer">
							...
							<NGrid class={`layer-grid`} cols={totalSpan}>
								{slotArr.map((slot, i) => {
						...
						{data.map((item, index) => {
							return (
								<div class={`yuan-table-layer body-layer ${rowClass}`}>
									...
									<NGrid class={`layer-grid`} cols={totalSpan}>
										{slotArr.map((slot) => (
											<NGridItem class="grid-item" span={slot!.props!.span || 1}>
												{slot}
											</NGridItem>
										))}
									</NGrid>
...
export const YuanTableColumn = defineComponent({
	...
	setup(props, { slots, emit }) {
		const { parent, index } = useParent<ParentExtraData>(YUAN_TABLE_KEY);
		...
		return () => {
			...
			return <div class="yuan-table-column">{slots.default!(data[dataIndex])}</div>;
		};
	},
});
