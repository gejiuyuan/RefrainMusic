...
export type OnRouteHookGuard = (
	to: RouteLocationNormalized,
	from: RouteLocationNormalized,
) => void;
export type OnRouteHookFilter = (
	to: RouteLocationNormalized,
	from: RouteLocationNormalized,
) => boolean;
...
export const onFilteredBeforeRouteUpdate: (guard: OnRouteHookGuard) => void = (
	updateGuard,
) => {
	onRouteHook(RouteHookNames.beforeUpdate, updateGuard, (to, from) => {
		...
		return (
			JSON.stringify(newToQuery) !== JSON.stringify(newFromQuery) || to.path !== from.path
		);
	});
};
