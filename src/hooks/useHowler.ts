...
export const defaultHowlOptions: Partial<HowlOptions> = {
	autoplay: false,
	html5: true,
...
const useHowler = (() => {
	...
	return (baseOptions: HowlOptions = defaultHowlOptions as Required<HowlOptions>) => {
		const playSound = (
			options: HowlOptions = defaultHowlOptions as Required<HowlOptions>,
		) => {
			//先销毁前面的实例
			Howler.unload();
