...
export const playingRefGlobal = (() => {
	let playing =
		localStorage.getItem(PreferenceNames.playing) === 'true' ||
		defaultAudioPreferences[PreferenceNames.playing];
...
export const currentTimeRefGlobal = (() => {
	let currentTime =
		Number(localStorage.getItem(PreferenceNames.currentTime)) ||
		defaultAudioPreferences[PreferenceNames.currentTime];
...
export const rateRefGlobal = (() => {
	let rate =
		Number(localStorage.getItem(PreferenceNames.rate)) ||
		defaultAudioPreferences[PreferenceNames.rate];
	return customRef<number>((track, trigger) => {
		return {
			get() {
				track();
...
export const volumeRefGlobal = (() => {
	let volume =
		Number(localStorage.getItem(PreferenceNames.volume)) ||
		defaultAudioPreferences[PreferenceNames.volume];
	return customRef<number>((track, trigger) => {
		return {
			get() {
				track();
...
export const orderRefGlobal = (() => {
	let order = (localStorage.getItem(PreferenceNames.order) ||
		defaultAudioPreferences[PreferenceNames.order]) as Order;
		...
		return {
			...
			set(value) {
				if (!orderOptions.includes(value)) {
					console.error(
						`The 'order' must be one of the three options:${orderOptions.join('、')}`,
					);
					return;
				}
				order = value;
...
export const muteRefGlobal = (() => {
	let mute =
		localStorage.getItem(PreferenceNames.mute) === 'true' ||
		defaultAudioPreferences[PreferenceNames.mute];
	return customRef<boolean>((track, trigger) => {
		return {
			get() {
				track();
...
on('loaderror', () => {
	...
	messageBus.dispatch(
		'errorMessage',
		`歌曲加载失败啦~将在${failureDuration / 1000}秒后播放下一首喔~${
			UNICODE_CHAR.pensive
		}`,
		{
...
export class AudioMaster extends EventDispatcher {
	...
	static resetAudioStatus() {
		durationRefGlobal.value =
			currentTimeRefGlobal.value =
			nextSeekTimeRefGlobal.value =
				0;
	}
