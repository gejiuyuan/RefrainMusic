...
export const theme = (() => {
	let theme =
		localStorage.getItem(PreferenceNames.theme) ||
...
export const playerQueueShow = (() => {
	let playerQueueShow =
		localStorage.getItem(PreferenceNames.playerQueueShow) === 'true' ||
		defaultPlayerPreferences[PreferenceNames.playerQueueShow];
...
const usePlayerStore = defineStore({
	...
	actions: {
		...
		handlePlaySoundNeededData(
			...
			getMusicDetail({ ids: String(id) }).then(({ songs: [songDetailData] }) => {
				...
				if (!queueSongList.some(({ id: queueSongId }) => id === queueSongId)) {
					const currentSongIndex = queueSongList.findIndex(
						({ id: queueSongId }) => currengSongId === queueSongId,
					);
		...
		isVideoBeLiked(videoId: string | number, description?: string) {
			return this.video.beLiked.some(
				({ mlogBaseData: { id, text, desc } }) =>
					id == videoId || description?.includes(desc),
			);
		},
