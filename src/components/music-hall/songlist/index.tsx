...
import { shallowReactive, getCurrentInstance, defineComponent } from 'vue';
...
import './index.scss';
import { NGrid, NGridItem, NDropdown, NButton, NxButton, NIcon } from 'naive-ui';
...
export default defineComponent({
	...
	setup(props, context) {
		...
		const getSonglist = async ({
			...
			listExtraInfo.order = orders.find(({ key }) => key === order)?.text || defaultOrder;
					...
					<section class="songlist-head">
						<NGrid>
							...
							<NGridItem span={2} offset={18}>
								...
								>
									<NxButton type="primary" size="tiny" iconPlacement="right" dashed ghost>
										{{
											default: () => listExtraInfo.order,
											icon: () => <i className="iconfont icon-xiajiantou"></i>,
										}}
									</NxButton>
								</NDropdown>
							</NGridItem>
