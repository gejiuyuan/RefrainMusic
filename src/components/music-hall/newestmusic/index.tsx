...
import { defineComponent, watch, WatchStopHandle, onActivated, reactive, ref } from 'vue';
	...
	setup(props, context) {
		...
		const routeUpdateHandler = async ({ query: { area = defaultAreaKey } }) => {
			...
			newestMusicInfo.listMap[newestMusicInfo.currentAreaKey] ??= (
				await getNewExpressMusic({ type: area })
			).data;
		};
			...
			return (
				<section class="music-hall-newestmusic">
					<section class="newestmusic-layer">
						<NRadioGroup value={currentAreaKey} onUpdateValue={areaChange} size="small">
					...
					<section class="newestmusic-layer">
						{willShowMusicList && (
							<MusicList musiclists={willShowMusicList} category="newest"></MusicList>
						)}
					</section>
				</section>
			);
