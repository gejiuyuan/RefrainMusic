	...
	setup(props, { slots, emit }) {
		...
		const locatoToCurrentSong = () => {
			...
			const targetScrollTop = (
				listContentUlRef.value!.children[targetIndex] as HTMLLIElement
			).offsetTop;
			const { scrollTop, offsetHeight } = listBodyElm.value!;
			//如果目标在可视区域内，就return
			if (targetScrollTop >= scrollTop && targetScrollTop < scrollTop + offsetHeight) {
				return;
			}
		...
		return () => {
			...
			return (
				<aside
					class="player-queue"
					slideShow={playerQueueShow.value}
					ref={playerQueueRef}
				>
					...
					<section class="queue-main">
						...
						<div
							class="locate-to-current"
							title="定位到当前播放歌曲"
							onClick={locatoToCurrentSong}
						>
