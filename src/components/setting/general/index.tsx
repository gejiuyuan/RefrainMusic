...
const themeConfig = {
  ...
  options: [
    ...
    '#18A058', '#2080F0', '#F0A020', 'rgba(208, 48, 80, 1)'
  ]
}
  ...
  setup(props, context) {
    ...
    const themeColorPickShowToggle = (isShow: boolean) => {
      if (!isShow) {
        ...
        setTimeout(() => {
          colorPickClickListenerRemover = useEventListener(document.querySelector('.n-color-picker-panel .n-color-picker-action button'), 'mouseup', handleThemePickerConfirm);
        });
      }
    }
