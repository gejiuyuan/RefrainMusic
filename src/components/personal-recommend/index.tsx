...
export const PersonalFm = defineComponent({
	...
	setup(props, { slots, emit }) {
		...
		watchEffect(async () => {
			picMainColor.value = await getImageMainColorString(
				currentVisibleFM.value?.album?.picUrl,
			);
		});
								...
								<div className="fm-main" aspectratio="3">
									...
									<div className="fm-content">
										...
										<div className="fm-tools">
											<div
												className="tool-item next-music"
...
export default defineComponent({
	...
	setup(props, { slots, emit }) {
		...
		const getPersonalRecommendMusics = async () => {
			const {
				data: { orderSongs, recommendReasons, dailySongs },
			} = await musicRecommend();
