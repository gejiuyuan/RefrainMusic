...
export type LrcItemInfo = {
  ...
  curIndex: number; //当前显示歌词的index索引
};
...
export default defineComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    const lrcItemInfo = reactive<LrcItemInfo>({
      heightArr: [],
      lastHeight: 0,
      firstHeight: 0,
      curIndex: 0,
    ...
    watch(() => audioStore.currentTime, (val) => {
    ...
    watch(() => suspension.isShow, (val) => {
      //浮动播放条显示时
      setLrcWrapTransition(!val);
    })

    //歌词列表变动时
    watch(() => playerStore.lyricParsed.lrcArr, async (val, oldVal) => {
      //如果没有歌词
      if (!playerStore.lyricParsed.exist) {
        //重置本歌词组件的一些数据
        resetLrcData();
        return;
      }
    ...
    watch(showTranslation, async (val) => {
      await nextTick();
      updatedLrcData();
      setTranslateYByDynamicIndex(suspension.tarIndex);
    })
    ...
    const setTranslateY = (value: number) => {
      lrcWrap.translateY.value = value;
    }
    ...
    const getTarTranslateY = (index: number) => {
      const heightArr = lrcItemInfo.heightArr;
      return (
        -heightArr
          .slice(0, index + 1)
    ...
    const getTarByTranslateY = () => {
      const curTranslateY = lrcWrap.translateY.value;
      const heightArr = lrcItemInfo.heightArr;
      const len = heightArr.length;
      const lastIndex = len - 1;
      const offsetY = Math.abs(
        curTranslateY - lrcWrap.translateY.max
      );
      let mutipleDis = 0;
      let index = 0;
      for (let i = 0; i < len; i++) {
    ...
    const setSuspensionHiddenTimer = (interval: number) => {
      suspension.hiddenTimer = setTimeout(() => {
        setSuspensionShow(false);
        //设置lrcWraptranslateY
        setTranslateYByDynamicIndex(lrcItemInfo.curIndex);
        //定时器结束后将拖动的目标index与curIndex同步，以防不断点击翻译按钮出错。
        setSuspensionTarIndex(lrcItemInfo.curIndex);
      }, interval);
    }
    ...
    const updatedLrcData = () => {
      ...
      lrcWrap.translateY = {
        min: -(lrcWrapHeight - lrcLastItemHeight / 2),
        max: -lrcfirstItemHeight / 2,
        value: -lrcfirstItemHeight / 2,
      };
    }
    ...
    const lyricMove = (ev: MouseEvent) => {
      const { canMove, translateY: lrcWrapTranslateY, rateFactor } = toRefs(
        lrcWrap
      );
      if (!canMove.value) return;
      const { min, max, value } = lrcWrapTranslateY.value;
      let translateY = value + ev.movementY * rateFactor.value;
      if (translateY > max) {
        translateY = max;
      } else if (translateY < min) {
        translateY = min;
      }
      lrcWrap.translateY.value = translateY;
      updateSuspensionPlayInfo();
    }
            ...
            <div class="exsit-operator">
              <div class="operator-textarea" onMousedown={lyricDown}>
                ...
                <ul
                  ...
                  ref={lrcContentsRef}
                  style={{
                    transform: `translateY(${lrcWrap.translateY.value}px)`,
