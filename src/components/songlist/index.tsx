...
import { NAvatar, NxButton, NMenu } from 'naive-ui';
...
export default defineComponent({
	...
	setup(props, context) {
		...
		const renderRelativeRecommendSonglist = () => {
			return (
				<section class="songlist-relative-playlist">
					<h5 class="relative-title">相关歌单推荐 {UNICODE_CHAR.smile}</h5>
					<Songlist
						playlists={songlistInfo.relativeRecommendSonglist}
		...
		return () => {
			...
			const {
				coverImgUrl,
				creator: { userId, detailDescription, avatarDetail, avatarUrl, nickname },
			} = playlist;
			...
			return (
				<section class="songlist-page">
					<section class="songlist-header">
						...
						<section class="songlist-header-body">
							...
							<section class="songlist-tools">
								<div class="songlist-btn" title={`分享量${playlist.shareCountStr}`}>
									<NxButton type="warning" size="small" ghost dashed>
										分享量：
										{playlist.shareCountStr}
									</NxButton>
								</div>
								<div class="songlist-btn" title={`订阅量${playlist.subscribedCountStr}`}>
									<NxButton type="warning" size="small" ghost dashed>
										订阅量：
					...
					<section class="songlist-main">{renderKeepAliveRouterView()}</section>
