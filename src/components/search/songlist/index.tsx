				...
				<section class="search-songlist">
					{playlists.map(
						({
							...
							<div class="search-songlist-item" onClick={() => toPlaylistDetailPage(id)}>
								<img loading="lazy" src={coverImgUrl} alt="" />
								<em class="songlist-name">{`${name}`}</em>
								<em class="songlist-trackcount">{`${getLocaleCount(trackCount)}首`}</em>
								<em class="songlist-creator" singallinedot>
									{`by  `}
									<span onClickCapture={() => toCreatorDetailPage(creator.userId)}>
										{creator.nickname}
									</span>
								</em>
								...
								<em class="songlist-listen">{`收听：${getLocaleCount(playCount)}`}</em>
							</div>
						),
					)}
