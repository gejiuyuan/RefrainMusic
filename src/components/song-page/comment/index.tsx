...
const CommentItemComp = defineComponent({
  ...
  setup(props) {
    ...
    const commentLikeHandler = async () => {
      const { commentItem: { commentId, liked }, sourceId, commentType  } = props;
      ...
      if(isSuccess) {
        ...
        messageBus.dispatch('successMessage', `${unOrLike ? '点赞成功!!' : '取消点赞成功~~'}${UNICODE_CHAR.smile}`)
      }
    }
    ...
    const renderCommentLike = () => {
      ...
      return (
        <span class={`span-item comment-like-span ${likedClass}`} onClick={commentLikeHandler}>
    ...
    return () => {
      const { commentItem: { user, time, content } } = props;
          ...
          <div class="comment-category-layer">
            <h4 class="category-headline">精彩评论</h4>
            {
              hotComments.map((data) => {
                return <CommentItemComp commentItem={data} sourceId={musicId} commentType={CommentType.music}></CommentItemComp>
              })
            }
          </div>
          <div class="comment-category-layer">
