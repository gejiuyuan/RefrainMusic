...
export default defineComponent({
	...
	setup(props, context) {
		...
		const vidWatcher = watch(
			() => route.query.vid as string,
			(vid) => {
				getRelativeVideos({
					id: vid,
				}).then(({ data }) => {
		...
		watchEffect(() => {
			videoData.isCollected = playerStore.isVideoBeLiked(
				videoData.detail.vid,
				videoData.detail.description,
			);
		});
		...
		const collectVideoHandler = async (value: boolean) => {
			...
			if (isSuccess) {
				...
				videoData.detail.subscribeCount =
					videoData.detail.subscribeCount + (value ? 1 : -1);
		...
		const praiseVideoHandler = async (value: boolean) => {
			...
			if (isSuccess) {
				...
				videoData.relativeInfo.likedCount =
					videoData.relativeInfo.likedCount + (value ? 1 : -1);
				messageTopic = 'successMessage';
				value && (msg = '点赞成功~~');
			}
			messageBus.dispatch(messageTopic, msg);
		};
		...
		const renderVideoOperator = () => {
			const {
				detail,
				isCollected,
				relativeInfo: { liked, likedCount, shareCount },
			} = videoData;
			const {
				title,
				publishTime,
				videoGroup,
				subscribeCount,
				creator: { avatarUrl, userId, nickname, followed },
			} = detail;
					...
					<div class="video-tags">
						<NSpace>
							{videoGroup.map(({ id, name }) => {
								return (
									<NxButton size="small" onClick={() => toVideoTagListPage(id)}>
										{name}
									</NxButton>
								);
							})}
					...
					<div className="video-buttons">
						<NSpace size={30}>
							<YuanButton
								...
								onUpdateIsActive={(value) => collectVideoHandler(value)}
							>
							...
							<YuanButton
								isActive={liked}
								onUpdateIsActive={(value) => praiseVideoHandler(value)}
							>
				...
				<section class="video-page">
					<div className="video-content">
						...
						<div className="video-recommend">
							<h4>相关推荐</h4>
							<VideoList
								videoList={recommendVideos.value}
								cols={4}
								gaps={{ x: 20, y: 20 }}
							></VideoList>
						</div>
