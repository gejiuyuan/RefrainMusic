...
export default defineComponent({
	...
	setup(props, context) {
		...
		watch<[string, VideoTagItem[]]>(
			() => [route.query.id as string, categoryVideosInfo.tagList],
			([routeId, tagList]) => {
				const realRouteId = +routeId;
				const targetTagIndex = is.undefined(routeId)
					? 0
					: tagList.findIndex(({ id }) => id === realRouteId);
		...
		const renderVideoTagList = () => {
			...
			return (
				<NSpace align="center" size={20}>
					{categoryVideosInfo.tagList.map(({ id, name }, i) => {
						return (
							<NxButton
								size="small"
								key={id}
		...
		return () => {
			return (
				<section class="online-video-category">
					{renderVideoTagList()}
